...
class ACSService implements ACSServiceInterface
{
    ...
    public function statistics($request)
    {
        ...
        $result[] = [
            ...
            'value' => round($tmp->where('treatment_result', 'Выписан в тяжелом состоянии')->count() / $n * 100,1)
        ];

        $tmp = $data;
        $result[] = [
            'title' => 'Доля пациентов с исходом в ОИМ с Q',
            'value' => round($tmp->where('final_result', 'ОИМ с Q')->count() / $n * 100,1)
        ];

        $tmp = $data;
        $result[] = [
            'title' => 'Доля пациентов с исходом в ОИМ без Q',
            'value' => round($tmp->where('final_result', 'Оим без Q')->count() / $n * 100,1)
        ];

        $tmp = $data;
        $result[] = [
            'title' => 'Доля пациентов с исходом в прогрессирующую стенокардию',
            'value' => round($tmp->where('final_result', 'Прогрессирующая стенокардия')->count() / $n * 100,1)
        ];

        $tmp = $data;
        $result[] = [
            'title' => 'Доля пациентов, поступивших в сроки до 6 часов',
            'value' => round($tmp->where('anginal_attack_date', 'до 6ч.')->count() / $n * 100,1)
        ];

        $tmp = $data;
        $result[] = [
            'title' => 'Доля пациентов, поступивших в сроки 6-12 часов',
            'value' => round($tmp->where('anginal_attack_date', '6-12ч.')->count() / $n * 100,1)
        ];

        $tmp = $data;
        $result[] = [
            'title' => 'Доля пациентов, поступивших в сроки позже 12 часов',
            'value' => round($tmp->where('anginal_attack_date', 'позже 12ч.')->count() / $n * 100,1)
        ];

        $tmp = $data;
        $result[] = [
            'title' => 'Доля пациентов, которым показана экстренное ЧКВ',
            'value' => round($tmp->where('cta_invasive_angiography', 'Да')->count() / $n * 100,1)
        ];

        $tmp = $data;
        $result[] = [
            'title' => 'Доля пациентов, которым показана отсроченное ЧКВ',
            'value' => round($tmp->where('cta_invasive_angiography' ,'Нет')->count() / $n * 100,1)
        ];
//dd($tmp->where('cta_invasive_angiography' ,'Нет')->count());
        $tmp = $data;
        $tmp1 = $data;
        ...
        $result[] = [
            ...
            'value' => round($tmp1->where('deferred_cta_invasive', 'Да')->count() > 0 ? $tmp->where('deferred_cta_completed', 'Да')->count() / $tmp1->where('deferred_cta_invasive', 'Да')->count() * 100 : 0,1)
        ];
        //14
        $tmp = $data;

        $numerator = $tmp->where('cta_90min', 'Да')->count() + $tmp->where('deferred_cta_completed', 'Да')->count();
        $denominator = $tmp->where('deferred_cta_invasive', 'Да')->count() + $tmp->where('cta_invasive_angiography', 'Да')->count();
        $isDenominatorPositive = $denominator > 0;
        $value = round($isDenominatorPositive ? ($numerator / $denominator) * 100 : 0,1);
        $result[] = [
